...
class FakeClient {
  signInImpl = vi.fn(
    async (_e: string, _p: string): Promise<AuthSession> => makeSession(),
  );
  signUpImpl = vi.fn(
  ...
  refreshImpl = vi.fn(
    async (_r: string): Promise<AuthSession> =>
  ...
  getUserImpl = vi.fn(async (_t: string) => ({
    id: "user_1",
    email: "e@x.com",
  }));

  signIn(email: string, password: string): Promise<AuthSession> {
    return this.signInImpl(email, password);
  }
  signUp(email: string, password: string): Promise<SignUpResult> {
    return this.signUpImpl(email, password);
  }
  refresh(refresh: string): Promise<AuthSession> {
    return this.refreshImpl(refresh);
  }
  signOut(token: string): Promise<void> {
    return this.signOutImpl(token);
  }
  recoverPassword(email: string): Promise<void> {
    return this.recoverImpl(email);
  }
  getUser(token: string): Promise<{ id: string; email: string }> {
    return this.getUserImpl(token);
  }
}
...
describe("auth facade", () => {
  ...
  it("signIn persists session, emits, returns view", async () => {
    ...
    const client =
      new FakeClient() as unknown as import("./client.js").AuthClient;
    const emitter = new EventEmitter();
    const events: unknown[] = [];
    emitter.on("session-changed", (v) => events.push(v));
    const view = await signIn("e@x.com", "pw", {
      client,
      reader,
      writer,
      emitter,
  ...
  it("signIn surfaces AuthError from client", async () => {
    ...
    fake.signInImpl.mockRejectedValueOnce(
      new AuthError("invalid_credentials", 400, "bad"),
    );
    await expect(
      signIn("e@x.com", "pw", {
  ...
  it("signOut still clears storage if /logout fails", async () => {
    ...
    const fake = new FakeClient();
    fake.signOutImpl.mockRejectedValueOnce(
  ...
  it("recoverPassword rejects empty email", async () => {
    await expect(recoverPassword("")).rejects.toMatchObject({
      code: "invalid_input",
    });
  });
