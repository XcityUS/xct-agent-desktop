...
export class AuthClient {
  ...
  async signUp(
    ...
    if (body && typeof (body as RawTokenResponse).access_token === "string") {
      return {
        kind: "session",
        session: parseSession(body as RawTokenResponse),
      };
    }
    ...
    throw new AuthError(
      "unknown",
      200,
      "signUp: unexpected response shape",
      body,
    );
  }
...
function mapErrorCode(
  ...
  if (haystack.includes("refresh") && haystack.includes("not found"))
    return "refresh_failed";
  if (
    status === 400 &&
    (haystack.includes("invalid_grant") || errCode === "invalid_credentials")
  ) {
  ...
  if (
    (haystack.includes("weak") || haystack.includes("password")) &&
  ...
  if (
    ...
    haystack.includes("already_registered")
  ) {
