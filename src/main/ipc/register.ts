...
export function registerIpcHandlers(context: IpcContext): void {
  ...
  // Xcity wallet (central xct-wallet at https://wallet.xcity.one).
  ...
  // Xcity auth (GoTrue at auth.xcity.one).
  ...
  ipcMain.handle("auth-refresh", async () => {
    try {
      const token = await authApi.refreshIfNeeded();
      return { ok: true as const, signed_in: Boolean(token) };
    } catch (e) {
      return mapAuthError(e);
    }
  });
  ...
  authApi.on("session-changed", () => {
    BrowserWindow.getAllWindows().forEach((window) => {
      window.webContents.send("auth-session-changed", authApi.getSessionView());
    });
  });
