...
export function startMainProcess(): void {
  ...
  app.on("second-instance", (_event, argv) => {
    const url = argv.find((arg) =>
      arg.startsWith(`${authApi.CUSTOM_PROTOCOL}://`),
    );
  ...
  authApi
    ...
    .then(() => {
      ...
      if (window) {
        if (window.isMinimized()) window.restore();
        window.focus();
        window.webContents.send("auth-oauth-completed", { ok: true });
      }
    })
    .catch((error: unknown) => {
      const code =
        error instanceof authApi.AuthError ? error.code : "unknown";
      const message = error instanceof Error ? error.message : String(error);
      const window = BrowserWindow.getAllWindows()[0];
      if (window) {
        window.webContents.send("auth-oauth-completed", {
          ok: false,
          code,
          error: message,
        });
